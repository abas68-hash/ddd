...
def getMI(ROIbox):
    ...
    def calculate_coordinate_moments():
        ...
        for i in range(xdim):
            ...
            # X moments
            m100 += x_weight * np.sum(slice_hold)
            om100 += x_weight * np.sum(slice_mask)
            ...
            for j in range(ydim):
                ...
                m010 += y_weight * np.sum(line_hold)
                om010 += y_weight * np.sum(line_mask)
                
                # Z moments
                m001 += np.sum(z_range * line_hold)
                om001 += np.sum(z_range * line_mask)
    ...
    def calculate_higher_moments():
        ...
        for i in range(xdim):
            ...
            for j in range(ydim):
                ...
                # Accumulate moments
                moments['u200'] += x_diff2 * np.sum(line_hold)
                moments['u020'] += y_diff2 * np.sum(line_hold)
                moments['u002'] += np.sum(z_diff2 * line_hold)
                
                moments['u110'] += x_diff * y_diff * np.sum(line_hold)
                moments['u101'] += x_diff * np.sum(z_diff * line_hold)
                moments['u011'] += y_diff * np.sum(z_diff * line_hold)
                
                moments['u300'] += x_diff3 * np.sum(line_hold)
                moments['u030'] += y_diff3 * np.sum(line_hold)
                moments['u003'] += np.sum(z_diff3 * line_hold)
                
                moments['u210'] += x_diff2 * y_diff * np.sum(line_hold)
                moments['u201'] += x_diff2 * np.sum(z_diff * line_hold)
                moments['u120'] += x_diff * y_diff2 * np.sum(line_hold)
                moments['u102'] += x_diff * np.sum(z_diff2 * line_hold)
                moments['u021'] += y_diff2 * np.sum(z_diff * line_hold)
                moments['u012'] += y_diff * np.sum(z_diff2 * line_hold)
                moments['u111'] += x_diff * y_diff * np.sum(z_diff * line_hold)
                
                # Mask moments
                omoments['ou200'] += ox_diff2 * np.sum(line_mask)
                omoments['ou020'] += oy_diff2 * np.sum(line_mask)
                omoments['ou002'] += np.sum(oz_diff2 * line_mask)
                
                omoments['ou110'] += ox_diff * oy_diff * np.sum(line_mask)
                omoments['ou101'] += ox_diff * np.sum(oz_diff * line_mask)
                omoments['ou011'] += oy_diff * np.sum(oz_diff * line_mask)
                
                omoments['ou300'] += ox_diff3 * np.sum(line_mask)
                omoments['ou030'] += oy_diff3 * np.sum(line_mask)
                omoments['ou003'] += np.sum(oz_diff3 * line_mask)
                
                omoments['ou210'] += ox_diff2 * oy_diff * np.sum(line_mask)
                omoments['ou201'] += ox_diff2 * np.sum(oz_diff * line_mask)
                omoments['ou120'] += ox_diff * oy_diff2 * np.sum(line_mask)
                omoments['ou102'] += ox_diff * np.sum(oz_diff2 * line_mask)
                omoments['ou021'] += oy_diff2 * np.sum(oz_diff * line_mask)
                omoments['ou012'] += oy_diff * np.sum(oz_diff2 * line_mask)
                omoments['ou111'] += ox_diff * oy_diff * np.sum(oz_diff * line_mask)
